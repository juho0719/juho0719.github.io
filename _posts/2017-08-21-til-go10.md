...
`os.Args` 사용
옵션 양식은 `-키=밸류` 형태
`flag` 패키지를 사용하여 편리하게 사용 가능

에러 처리
`fmt.Errorf("%d개의 에러", n)`

Fatal() : 에러 로그를 출력하고 완전히 종료
Panic() : 시간과 에러 문자열을 출력하고 패닉 발생
Print() : 시간과 에러 메시지를 출력하고 프로그램을 종료하지 않음

단위 테스트
`import "testing"` 
파일이름은 _test.go를 붙임
함수명은 Test로 시작해야함
Test다음에 첫글자는 반드시 대문자
실행 방법 : `go test` 

밴치마크 테스트
성능 측정 기능
함수명은 Benchmark로 시작
Benchmark 다음 첫글자는 반드시 대문자
실행 방법 : `go test -bench` 
